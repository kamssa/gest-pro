...
import {
  RetraitBanqueActions,
  BanqueActionsTypes,
  GetAllBanqueActionSuccess,
  GetAllBanqueError,
  SaveBanqueActionError,
  SaveBanqueActionSuccess
} from './retrait-banque.actions';


@Injectable()
export class RetraitBanqueEffects {
  id: number;
  personne: any;
  ...
  getAllanqueEffect: Observable<RetraitBanqueActions> = createEffect(
    () => this.effectActions.pipe(
      ofType(BanqueActionsTypes.GET_ALL_BANQUE),
      mergeMap((action: RetraitBanqueActions) => {
    ...
    saveBanqueEffect: Observable<RetraitBanqueActions> = createEffect(
    () => this.effectActions.pipe(
